...
function parseHtmlFile(filePath: string, slug: string): { title: string; headContent: string; blocks: ParsedBlock[] } {
  ...
  console.log('V2 detected blocks:', blocks.map(b => ({ type: b.type, draggable: b.draggable, fields: b.fields.length })));
...
function divideBodyIntoBlocks(bodyElement: any, htmlContent: string): ParsedBlock[] {
  ...
  console.log('V2 Manual blocks found:', manualBlocks.map(b => `${b.type}:${b.draggable}`));
...
async function importHtmlFiles() {
  ...
  try {
    ...
    for (const file of files) {
      ...
      console.log(`Importing ${file} with V2 approach...`);
      ...
      // Aggiungi suffisso v2 per non sovrascrivere
      const v2Slug = `${slug}-v2`;
      ...
      console.log(`✓ Imported ${v2Slug} with ${blocks.length} blocks using template approach`);
    }
    
    console.log('\n🎉 V2 HTML import completed!');
  } catch (error) {
